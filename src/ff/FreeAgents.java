...
public class FreeAgents {
	...
	private ArrayList<String> playerFirstNames;
	
	private ArrayList<String> playerLastNames;
	
	private ArrayList<String> playerNames = new ArrayList<String>();
	
	private ArrayList<String> playerTeams;
	private ArrayList<String> playerTeamsDefense;
	...
	public void generateFreeAgents() {
		...
		playerFirstNames = new ArrayList<>(Arrays.asList("Liam", "Noah", "Oliver", "Elijah",
		...
		playerLastNames = new ArrayList<>(Arrays.asList("Smith", "Johnson", "Williams", "Brown",
				...
				"Richardson", "Cox", "Howard", "Ward", "Torres", "Peterson", "Gray")); // add 120 last names
		
		playerTeams = new ArrayList<>(Arrays.asList("Arizona", "Atlanta", "Baltimore", 
				...
				"New England", "Seattle", "New Jersey", "Tampa Bay", "Pittsburgh", "Washington", "Tennessee")); // create list of teams
		
		playerTeamsDefense = new ArrayList<>(Arrays.asList("Arizona", "Atlanta", "Baltimore", 
				"Buffalo", "Carolina", "Chicago", "Cincinnati", "Cleveland", "Dallas", "Denver", "Detroit", 
				"Houston", "Green Bay", "Indianapolis", "Los Angeles", "Jacksonville", "Minnesota", "Kansas City", 
				"New Orleans", "Las Vegas", "New York", "San Diego", "Philadelphia", "Miami", "San Francisco", 
				"New England", "Seattle", "New Jersey", "Tampa Bay", "Pittsburgh", "Washington", "Tennessee")); // create list of defensive teams
		
		for (int i = 0; i < 40; i++) { // add 40 QBs
			playerPositions.add("QB");
		}
		for (int i = 0; i < 66; i++) { // add 66 RBs
			playerPositions.add("RB");
		}
		for (int i = 0; i < 98; i++) { // add 98 WRs
			playerPositions.add("WR");
		}
		for (int i = 0; i < 48; i++) { // add 48 TEs
			playerPositions.add("TE");
		}
		for (int i = 0; i < 32; i++) { // add 32 Ks
			playerPositions.add("K");
		}
		
		Collections.shuffle(playerPositions);
		Collections.shuffle(playerFirstNames);
		Collections.shuffle(playerLastNames);
		Collections.shuffle(playerTeams);
		
		// create names
		for (int i = 0; i < 284; i++) {
			int namesIndex = random.nextInt(playerFirstNames.size());
			int namesIndex2 = random.nextInt(playerLastNames.size());
			playerNames.add(playerFirstNames.get(namesIndex) + " " + playerLastNames.get(namesIndex2));
		...
		// create players
		for (int i = 0; i < 284; i++) {
			int positionsIndex = random.nextInt(playerPositions.size());
			int namesIndex = random.nextInt(playerNames.size());
			int teamsIndex = random.nextInt(playerTeams.size());
			
			String position = playerPositions.get(positionsIndex);
			String name = playerNames.get(namesIndex);
			String team = playerTeams.get(teamsIndex);
			
			Player player = new Player(position, name, team);
			freeAgents.add(player);
			
			playerPositions.remove(positionsIndex);
			Collections.shuffle(playerPositions); // shuffle positions
			
			playerNames.remove(namesIndex);
			Collections.shuffle(playerNames); // shuffle names
			
			Collections.shuffle(playerTeams); // shuffle teams
		}
		
		// create defenses
		for (int i = 0; i < 32; i++) {
			int teamsIndex = random.nextInt(playerTeamsDefense.size());
			
			Player player = new Player("DEFENSE", playerTeamsDefense.get(teamsIndex));
			freeAgents.add(player);
			
			playerTeamsDefense.remove(teamsIndex);
			Collections.shuffle(playerTeamsDefense); // shuffle defenses
		}
	} // end method generate freeAgents
	
	public ArrayList<Player> getFreeAgents() {
		return freeAgents;
	} // end getFreeAgents
	
	public void removeFreeAgents(int index) {
		freeAgents.remove(index);
	} // end removeFreeAgents
	
	@Override
	public String toString() {
		String finalPlayerStr = "";
		for (Player player: freeAgents) {
			finalPlayerStr = finalPlayerStr + player.toString();
		}
		return finalPlayerStr;
	} // end toString

} // end class FreeAgents
